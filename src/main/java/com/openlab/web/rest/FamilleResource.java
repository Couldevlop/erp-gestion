...
/**
 * REST controller for managing {@link com.openlab.domain.Famille}.
 */
@RestController
@RequestMapping("/api")
public class FamilleResource {
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/familles/{id}", consumes = "application/merge-patch+json")
     ...
     */
    @GetMapping("/familles")
    public ResponseEntity<List<FamilleDTO>> getAllFamilles(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("article-is-null".equals(filter)) {
            log.debug("REST request to get all Familles where article is null");
            return new ResponseEntity<>(familleService.findAllWhereArticleIsNull(), HttpStatus.OK);
        }
        log.debug("REST request to get a page of Familles");
        Page<FamilleDTO> page = familleService.findAll(pageable);
